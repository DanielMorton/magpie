...
fn get_common_name(species: Option<&scraper::ElementRef>) -> String {
    species.and_then(|s| s.text().next())
        .map(|s| s.trim().to_owned())
        .unwrap_or_default()
}

fn get_scientific_name(species: Option<&scraper::ElementRef>) -> String {
    species.and_then(|s| s.select(selectors::target::sci_name()).next())
...
fn get_species(row: &ElementRef) -> Result<(String, String)> {
    let species = row.select(selectors::target::species())
        .next()
        .and_then(|s| s.select(selectors::target::a()).next());
    Ok((get_common_name(species.as_ref()), get_scientific_name(species.as_ref())))
}
...
pub fn scrape_table(table: ElementRef, checklists: i32) -> Result<DataFrame> {
    let rows: Result<Vec<DataFrame>> = table.select(selectors::target::rows())
        .map(|row| {
