...
pub fn hotspot_to_df(hotspots: &[Hotspot]) -> DataFrame {
    let hotspot = hotspots.iter().map(|h| h.hotspot.to_owned()).collect::<Vec<_>>();
    let hotspot_code = hotspots.iter().map(|h| h.hotspot_code.to_owned()).collect::<Vec<_>>();
    let sub_region = hotspots
        .iter()
        .map(|h| h.sub_region())
        .collect::<Vec<_>>();
